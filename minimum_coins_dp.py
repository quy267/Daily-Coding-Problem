...
This will run really slowly --
in exponential time, since for each call we're making up to 4 calls recursively and only decrementing our input by a constant value.
To improve performance, we can use dynamic programming to keep an of all of our previous results. 
Now we can just look up values in the array instead of repeating subcomputations.
"""
